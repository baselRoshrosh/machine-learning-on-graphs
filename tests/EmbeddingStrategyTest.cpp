...
TEST_F(EmbeddingStrategyTest, BasicSampling) {
    ...
    int sampleSize = 2;
    auto sample = embeddingStrategy->getSample(vectors, sampleSize);
...
TEST_F(EmbeddingStrategyTest, EmptyInput) {
    std::vector<std::vector<double>> vectors;
    auto sample = embeddingStrategy->getSample(vectors, 3);
...
TEST_F(EmbeddingStrategyTest, ZeroOrNegativeSampleSize) {
    ...
    auto sampleZero = embeddingStrategy->getSample(vectors, 0);
    auto sampleNegative = embeddingStrategy->getSample(vectors, -3);
